...
void naive_common_substring(const std::string& s1, const std::string& s2) {
    std::vector<std::string> ans;
    for(int i = 0; i < s1.size(); ++i) {
        for(int j = i; j < s2.size(); ++j) {
            if(s2.find(s1.substr(i, j)) != s2.npos) {
                ans.push_back(s1.substr(i, j));
            } 
        }
    }
...
int main() {
    ...
    uint64_t fast = std::chrono::duration_cast<duration_t>( end_ts - start_ts ).count();
    // // Измеряем время работы stl сортировки.
    start_ts = std::chrono::system_clock::now();
    std::cout << "Counting sort time: " << fast << DURATION_PREFIX << std::endl;

    naive_common_substring(s1, s2);
