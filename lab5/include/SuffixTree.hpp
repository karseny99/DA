...
public:
    ...
    void addSymbol(char symbol) {
        ...
        while(reminder > 0) {
            ...
            std::cout << reminder  << ' ' << currentNode->l << ' '<< currentNode->r <<std::endl;
            for(auto c : currentNode->nodes) {
                std::cout << static_cast<char>(c.first);
            }
            std::cout << ' ' << text[currentEdge] << ' ' << currentShift << std::endl;
